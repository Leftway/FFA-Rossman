...
# Remove store as it is not a predictive variable
train.store <- train.store %>% dplyr::select(-Store)  
View(train.store)

test.store <- test.store %>% dplyr::select(-c(Store, Id))
...
xgbModel <- xgb.cv(params=params,
                data=data.matrix(train.store),
                label=train.store$Sales,
                nrounds=5,
                eval_metric="rmse",
                nfold=10,
                stratified=TRUE)

numTrees <- min(which(xgbModel$evaluation_log$train_rmse_mean == 
                        max(xgbModel$evaluation_log$train_rmse_mean)))

xgbModel <- xgboost(params  = params,
                    data    = data.matrix(test.store),
                    label   = test.store$Sales,
                    nrounds = numTrees,
                    eval_metric = "rmse")
